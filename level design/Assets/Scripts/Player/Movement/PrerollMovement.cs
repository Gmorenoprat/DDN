...
public class PrerollMovement : IMovementMode
{
    Player _player;
    Rigidbody _rb;
    Transform _cam;
    ...
    public PrerollMovement(Transform cam, Player player, Rigidbody rb, float speed)
    {
        _player = player;
        _speed = speed;
        _rb = rb;
    ...
    public void Move(float v, float h)
    {
        ...
        _rb.velocity = (Z + Y + X);
        _player.transform.forward = (_cameraRight * h + _cameraForward * v).normalized;
